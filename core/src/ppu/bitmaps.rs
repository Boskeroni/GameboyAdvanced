...
pub fn bg_mode_3(layers: &mut LineLayers, memory: &mut Box<Memory>, line: u16){
    let start = 0x6000000 + (line as u32 * 480); // screen width * 2
    for i in 0..LCD_WIDTH {
        let pixel = memory.read_u16(start + i as u32 * 2);
        layers.bgs[2][i as usize] = pixel;
    }
}

// TODO: this isn't always displaying text for some reason, maybe not a mode_4 issue
pub fn bg_mode_4(layers: &mut LineLayers, memory: &mut Box<Memory>, line: u16) {
...
pub fn bg_mode_5(layers: &mut LineLayers, memory: &mut Box<Memory>, line: u16) {
